...
void simd_matmul(void) {
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < p; j+=4)
        {
            ...
            for (int k = 0; k < 4; ++k)
            {
                al = _mm_set1_ps(a[i][k]);
                bl = _mm_loadu_ps(b[k] + j);
                cl = _mm_add_ps(cl, _mm_mul_ps(al, bl)); 
            }
            _mm_storeu_ps(c2[i] + j, cl); 
        }
    }
...
void loop_unroll_simd_matmul(void) {
        ...
        for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < p; j+=4)
        {
            ...
            al = _mm_set1_ps(a[i][0]);
            bl = _mm_loadu_ps(b[0] + j);
            cl = _mm_add_ps(cl, _mm_mul_ps(al, bl)); 

            al = _mm_set1_ps(a[i][1]);
            bl = _mm_loadu_ps(b[1] + j);
            cl = _mm_add_ps(cl, _mm_mul_ps(al, bl)); 

            al = _mm_set1_ps(a[i][2]);
            bl = _mm_loadu_ps(b[2] + j);
            cl = _mm_add_ps(cl, _mm_mul_ps(al, bl)); 

            al = _mm_set1_ps(a[i][3]);
            bl = _mm_loadu_ps(b[3] + j);
            cl = _mm_add_ps(cl, _mm_mul_ps(al, bl)); 
