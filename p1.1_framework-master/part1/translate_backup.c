                   ...
                   size_t num_args, uint32_t addr, SymbolTable *symtbl)
{
    ...
    if (strcmp(name, "beqz") == 0) {
        ...
        /* Setup parameters */
        sub_args[0] = get_addr_for_symbol(symtbl, args[0]);
    ...
    else if (strcmp(name, "jal") == 0)
    {
        ...
        {   
            sub_args[0] = translate_reg("ra"); 
            sub_args[1] = get_addr_for_symbol(symtbl, args[0]);
        }
        else 
        {
            sub_args[0] = args[0];
            sub_args[1] = get_addr_for_symbol(symtbl, args[1]);
        }
        if(addr==-1)
            return 0;
    ...
    else if (strcmp(name, "jalr") == 0)
    {
        ...
        if(write_instruction(basic_code, machine_code, "jalr", sub_args, 2, addr, symtbl)==-1)
            return 0;
        return 1;
        /* YOUR CODE HERE */
    }
        ...
        addr = get_addr_for_symbol(symtbl, args[1]);
        if(addr==-1)
            return 0;
                ...
                uint8_t funct3, char **args, size_t num_args, SymbolTable *symbol)
{
    ...
    rs = translate_reg(args[1]);
    /* source register 2 */
    imm = translate_num(transnum_tmp, args[1], 0, 31);
                ...
                uint8_t funct3, char **args, size_t num_args, SymbolTable *symbol)
{
    ...
    /* generate instruction */
    instruction = opcode + (imm << 7) + (funct3 << 12) + (rs1 << 15) + (rs2 << 20) + (imm << 25);
