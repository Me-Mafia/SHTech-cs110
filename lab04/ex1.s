...
#1.simplefm: 使用了没用设定好的寄存器t0, not initialized
#naive：没有移动栈指针sp，为函数内的变量开空间
#inc： 没用完全移动，少开了空间(s0,s1),and failure to save t0 before calling helper_fn.
#2. no
#3. ra：用于保存函数调用返回地址，inc中调用helperfunction时，ra的值会被覆盖，因此要先存好
#4. helper not declared in .globl